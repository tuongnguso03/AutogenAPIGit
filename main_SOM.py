...
def initiate_agents(chat_history: list = []):
    ...
    speaker_transitions_dict = {code_assistant:[assistant, code_assistant]} #do not force user to repeat

    groupchat = GroupChat(
            agents=[assistant, code_assistant, executor], 
    ...
    for message in chat_history:
        user_proxy.send(message, society_of_mind_agent, request_reply=False, silent=True)


    return user_proxy, society_of_mind_agent

def retrieve_chat_history(chat_id: int) -> tuple[UserProxyAgent, SocietyOfMindAgent]:
    
    if os.path.exists(f'./chat_history/{chat_id}.json'):
        with open(f'./chat_history/{chat_id}.json', 'r') as f:
            chat_history = json.load(f)
            user_proxy, society_of_mind_agent = initiate_agents(chat_history)
            print(f"retrieve chat history of chat {chat_id}")
    else:
        user_proxy, society_of_mind_agent = initiate_agents()
    return user_proxy, society_of_mind_agent
...
def chatting(user_chat_line: UserChatLine):
    ...
    user_proxy, society_of_mind_agent = retrieve_chat_history(chat_id)

    chat_result = user_proxy.initiate_chat(society_of_mind_agent, False, True, message=message)
    save_dir = f'./chat_history/{chat_id}.json'
    with open(save_dir, 'w') as f:
        json.dump(chat_result.chat_history, f)
